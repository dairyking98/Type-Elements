...
//renders exactly one character of Physical_Layout at its (row,col) position -
//extracted from TextRing()'s loop body so a single glyph can be rendered in
//isolation (e.g. one glyph per CLI process, for parallel-render experiments -
//see v3/). Recomputes the small per-row metadata (_rowFont/_rowFontSize/etc)
//locally rather than taking them as parameters, so this module stays fully
//self-contained/independently callable - negligible cost (array
//comprehensions over row/col count, not geometry), doesn't affect output.
module RenderOneGlyph(row, col){
