        ...
        union(){ //Place Letters onto Blank Cylinder
            for (row=[0:1:len(Layout)-1]){
                for (n=[0:1:len(Layout[0])-1]){
                    ...
                    GlobalPosition(Min_Final_Character_Diameter/2, Baseline[row], Baseline_Offset[row], n, len(Layout[0]))
                    LetterText(Cutout[row], Cutout_Offset[row], Typeface_, Type_Size, Layout[row][PickedChar], Platen_Diameter, Min_Final_Character_Diameter, Draft_Angle, Debug_No_Minkowski, Horizontal_Weight_Adj, Vertical_Weight_Adj, Weight_Adj_Mode);
