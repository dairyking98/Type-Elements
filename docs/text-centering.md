...
### `Show_Reference_Line` — toggle the caption line on/off

A Customizer checkbox (declared right after `Test_String_Text`, defaulting to `true` so existing behavior is unchanged) that wraps the flat caption `text()` call in an `if()`. Off leaves only the embossed/CPI-spaced character row, for when the reference line isn't wanted (e.g. a cleaner preview screenshot, or a font/size where the two overlapping lines get visually noisy). Verified by rendering with it set `false` — only the embossed row remains.
