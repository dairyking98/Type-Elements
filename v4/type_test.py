#!/usr/bin/env python3
"""
Builds a flat, CPI-spaced test line for quick font/legibility testing -
...
not proportional spacing), centered within it. Independent of the real
cylindrical element pipeline (no draft, no platen scallop, no
placement-on-cylinder) - just flat extruded outlines, for speed.

Usage:
    python3 type_test.py "text to render" --cpi 10 --font-path /path/to/font.ttf --font-size-mm 3.7 --out output/blickensderfer_full_element.stl
"""
...
def build_type_test_line(text, cpi, font_path, font_size_mm, points_per_mm=8.0, depth=0.4):
    ...
    for i, ch in enumerate(text):
        ...
        parts.append(mesh)
    if not parts:
        raise ValueError("nothing to render - text was empty or all spaces")
