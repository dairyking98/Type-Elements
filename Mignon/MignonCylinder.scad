...
module LetterText(Char, row, column){
    minkowski(){
        difference(){
            ...
            rotate([0,90,0])
            cylinder(h=10,d=Platen_Diameter,center=true,$fn=cyl_fn);
        }
        if (Debug_No_Minkowski != true)
            translate([0, 0, -6])
            cylinder(h=6,r2=0,r1=.75*6, $fn=mink_fn);
    }
...
module TextRing(){
    for (row=[0:1:len(Layout)-1]){
        for (column=[0:1:len(Layout[0])-1]){
            ...
            echo(char=Layouts[5][row][PickedChar], baseline=Baseline[row]+ (testing_baseline==true?Testing_Offsets[column]:0), cutout=Cutout[row]+ (testing_cutout==true?Testing_Offsets[column]:0));
            }
        }
    }
}
...
translate([0, 0, -1])
    linear_extrude(Cylinder_Height+6)
    rotate([0,0,360/24])
...
module MinkCleanup(){
translate([0, 0, Cylinder_Height-Cylinder_Top_Height_Offset])
    cylinder(h=10, d=30);
    rotate([180, 0, 0])
    cylinder(h=10, d=30);
}
