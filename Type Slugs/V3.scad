...
difference(){
    ...
    //Lower Wing
    translate([-Body_Width/2-.001, -.001, -Body_Height*1.5])
    rotate([90, 0, 90])
    linear_extrude(Body_Width+.002)
    polygon([[0, 0], [0, Body_Height*1.5], [Body_Height*1.5*sin(Lower_Wing_Angle), 0]]);
    //Radius Lower Wing
    translate([-Body_Width/2-.001, (Body_Height-Wing_Radius)*sin(Lower_Wing_Angle), -Body_Height])
    rotate([90, 0, 90])
    linear_extrude(Body_Width+.002)
    difference(){
        square(Wing_Radius);
        translate([Wing_Radius, Wing_Radius])
        circle(r=Wing_Radius);
    }
    //Top Wing
//    translate([0, Body_Length, 0])
//    rotate([-90-Upper_Wing_Angle, 0, 0])
//    translate([-10, -10, 0])
//    cube([20, 20, 20]);
    
    translate([Body_Width/2+.001, Body_Length+.001, -Body_Height*1.5])
    rotate([90, 0, -90])
    linear_extrude(Body_Width+.002)
    polygon([[0, 0], [0, Body_Height*1.5], [Body_Height*1.5*sin(Upper_Wing_Angle), 0]]);
    //Radius Top Wing
    translate([Body_Width/2+.001, Body_Length-(Body_Height-Wing_Radius)*sin(Upper_Wing_Angle), -Body_Height])
    rotate([90, 0, -90])
    linear_extrude(Body_Width+.002)
    difference(){
        square(Wing_Radius);
    ...
    translate([0, 0, -.001]){
        ...
        polygon([[Body_Width/2, 0], [Body_Width/2+1, 0], [Body_Width/2+1, Body_Height+.001], [Body_Slot_Width/2+Wing_Thickness, Body_Height+.001]]);
        mirror([1, 0, 0])
        linear_extrude(Body_Length+.002)
        polygon([[Body_Width/2, 0], [Body_Width/2+1, 0], [Body_Width/2+1, Body_Height+.001], [Body_Slot_Width/2+Wing_Thickness, Body_Height+.001]]);
        }
