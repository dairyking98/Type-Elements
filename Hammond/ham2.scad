...
//Comic Mono//Layout=["-;p.lo,kimjunhybgtvfrcdexswzaq",
//        "!:P.LO?KIMJUNHYBGTVFRCDEXSWZAQ",
//        "/=0.)9•(8^'7*&6`_5>%4+$3<#2@\"1"];
...
Shuttle_Square_Hole_Offset=5.80;//6.47 is OEM measurement
//Square Hole Width
Shuttle_Square_Hole_Width=2.67;//2.54 OEM measurement + .13 printer offset
//Square Hole Length
Shuttle_Square_Hole_Length=1.20;//.01
...
module Rib(){
    translate([0, 0, rib_bottomz]){
        linear_extrude(Shuttle_Rib_Thickness){
            ...
            polygon([[Shuttle_Arc_Radius*cos(60), -Shuttle_Arc_Radius*sin(60)],[z_offset, 0],  [Shuttle_Arc_Radius*cos(60), Shuttle_Arc_Radius*sin(60)], [0, 100], [-100, 0], [0, -100]]);
            }
        }
    }
}
...
module PinSupportHull(){
    hull(){
        ...
        translate([0-Shuttle_Pin_Support_Base_Width/2+Shuttle_Square_Hole_Length/2+Shuttle_Pin_Support_HeightOffset, -Shuttle_Pin_Support_Base_Length/2])
        linear_extrude(z)
        RadiusSquare(Shuttle_Pin_Support_Base_Width, Shuttle_Pin_Support_Base_Length, Shuttle_Square_Hole_Radius, cyl_fn);
    }
}
        ...
        translate([0, 0, -Resin_Support_Min_Height-Resin_Support_Base_Thickness]){
            ...
            for (y=[-(round(30/Resin_Support_Spacing)+1)*Resin_Support_Spacing:Resin_Support_Spacing:(round(30/Resin_Support_Spacing)+1)*Resin_Support_Spacing]){
                ...
                for (x=[-x_min+Resin_Support_Contact_Diameter/2,-x_min*2/3, -x_min*1/3, x_max*1/3, x_max*2/3, x_max-Resin_Support_Contact_Diameter/2]){
                    ...
                    for (n=[0,1]){
                    ConnectingRod(
                        [[-x_min*2/3, x_max*2/3][n],y,h+z-Shuttle_Rib_Width],
                        [[-(x_min-Resin_Support_Contact_Diameter/2), (x_max-Resin_Support_Contact_Diameter/2)][n],y,h+z-Shuttle_Rib_Width-3],
                    Resin_Support_Rod_Thickness);
                    
                    ConnectingRod(
                        [[-x_min/3, x_max/3][n],y,h+z-Shuttle_Rib_Width],
                        [[-x_min*2/3, x_max*2/3][n],y,h+z-Shuttle_Rib_Width-3],
                    Resin_Support_Rod_Thickness);}
            ...
            //#hull(){
                ...
                for (x=[-x_min+Resin_Support_Contact_Diameter/2,-x_min*2/3, -x_min*1/3,0, x_max*1/3, x_max*2/3, x_max-Resin_Support_Contact_Diameter/2]){
...
module VertResinSupport2(){
    ...
    union(){
        //Under Large Arc
        thetamax=120*PI/180-2*Shuttle_Taper*PI/180;
        ...
        for (s=[-1,1]){
            for (x=[-x_min+Resin_Support_Contact_Diameter/2,-x_min*2/3, -x_min*1/3, x_max*1/3, x_max*2/3, x_max-Resin_Support_Contact_Diameter/2]){
                ...
                for (theta=[0:thetaspacing:thetamax/2]){
                    ...
                    for (n=[0,1]){
                    ConnectingRod(
                        [[-x_min*2/3, x_max*2/3][n],y,za-z_offset+Resin_Support_Base_Thickness+Resin_Support_Min_Height-2],
                    ...
                    ConnectingRod(
                        [[-x_min*1/3, x_max*1/3][n],y,za-z_offset+Resin_Support_Base_Thickness+Resin_Support_Min_Height-2],
                        [[-x_min*2/3, x_max*2/3][n],y,za-z_offset+Resin_Support_Base_Thickness+Resin_Support_Min_Height-2-3],
                    Resin_Support_Rod_Thickness);
                    }
                //}
            //}
